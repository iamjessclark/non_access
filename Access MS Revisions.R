...
for(i in 1:length(pops)){ 
  for(j in 1:nrow(Values)){
    ...
    while(HALT==FALSE){
      ...
      Prev20[[i]][j,month_index] <- (length(which(Clone$sysExProb==0 & Clone$age>=20*12 & Clone$Mf>0))/(length(which(Clone$sysExProb==0 & Clone$age>=20*12))))*100 # using MF not worms 
...
MDAroundstotal <- lapply(1:length(pops), matrix, data= NA, nrow=nrow(Values), ncol=2)
